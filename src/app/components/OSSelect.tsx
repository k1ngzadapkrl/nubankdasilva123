...
export default function OSSelect({ setSelectedOs, setView }: any) {
  return (
    <div className="fixed inset-0 bg-[#070707] flex flex-col justify-center p-8 z-25">
      <h2 className="text-white text-center mb-8 font-bold text-xl uppercase tracking-widest">
        Selecione o Sistema
      </h2>
      <div className="space-y-4">
        <button 
          onClick={() => { setSelectedOs('android'); setView('version_select'); }} 
        ...
        <button 
          onClick={() => { setSelectedOs('ios'); setView('version_select'); }} 
          className="w-full bg-[#111] p-6 rounded-2xl flex justify-between text-white font-bold border border-zinc-900 active:bg-[#1a1a1a]"
        >
