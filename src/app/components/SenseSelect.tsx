  ...
  if (showResult) {
    return (
      <div className="fixed inset-0 bg-[#0f0f0f] text-white p-6 z-50 overflow-y-auto">
        <button onClick={() => setShowResult(false)} className="mt-8 mb-6 text-zinc-500 text-sm flex items-center gap-2">
          <span>←</span> Voltar
        </button>
        ...
        <div className="mt-6 p-4 bg-[#ff6b00]/10 border border-[#ff6b00]/20 rounded-2xl text-center">
          <p className="text-[#ff6b00] text-[10px] font-bold uppercase tracking-widest">Ajuste de Sistema</p>
          <p className="text-white font-mono mt-1 text-sm">{currentSensi.extra}</p>
        </div>

        <div className="grid grid-cols-2 gap-3 mt-8">
          <button onClick={() => setShowResult(false)} className="bg-zinc-800 py-4 rounded-2xl font-bold text-xs uppercase active:scale-95 transition-all">Gerar de novo</button>
          <button className="bg-zinc-800 py-4 rounded-2xl font-bold text-xs uppercase active:scale-95 transition-all">Copiar tudo</button>
        </div>
        <button className="w-full bg-[#ff6b00] text-black font-bold py-4 rounded-2xl mt-3 uppercase text-xs shadow-lg active:scale-95 transition-all">Salvar Sensi</button>
      </div>
    );
  }
